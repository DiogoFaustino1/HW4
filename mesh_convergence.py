...
for iy in range(len(num_y_values)):
    ...
    surf_dict = {
        ...
        "span": 23.24,
        "twist_cp": np.array([0.0, 1.0, 2.0, 3.0]),  # [deg]
        #"chord_cp" : np.ones(10),
        ...
        # Create a dictionary to store options about the surface
    mesh_dict = {"num_y": num_y, "num_x": num_x, "wing_type": "rect", "symmetry": True, "offset": np.array([10, 0.0, 3.0])}
    ...
    #prob.model.add_design_var("wing.twist_cp", lower=np.array([[0, 0, 0, 0]]), upper=np.array([[0, 3, 6, 9]]), scaler=0.1)
